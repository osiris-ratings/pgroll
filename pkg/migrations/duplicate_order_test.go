...
// TestColumnDuplicationOrderIsDeterministic is the ENG-6193 regression guard.
//
...
// iteration order is randomized per process. So the recreated column order was
// non-deterministic across applications — the same migration could seal as
// `name, person_id` on one deploy and `person_id, name` on another, flaking
// the "Prove Migrations Revert Cleanly" CI job on `watchlist_person_hits`.
//
...
func TestColumnDuplicationOrderIsDeterministic(t *testing.T) {
	...
	tbl := &schema.Table{Name: "watchlist_person_hits", Columns: tableColumns}
	...
	// Each application builds a fresh map and ranges it once; 1000 independent
