...
- `create_constraint` is now classified as deferred in batched migrates (`CompleteMustBeDeferred`). It is a duplicator-pattern operation whose Complete drops the original user-facing columns and renames the duplicates back — running that inline mid-batch collides with the previous-production schema's view dependencies (`DROP COLUMN` is not `CASCADE`), the exact failure the defer mechanism exists for. Deferral also makes it losslessly revertible.
